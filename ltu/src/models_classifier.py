...
class MLPClassifier:
    """Basic construction of a classifier."""
    name = 'mlp classifier'
    ...
    def fit(self, x_train, y_train, x_val, y_val):
        ...
        x_train_loader = DataLoader(
            x_train, batch_size=self.batch_size, shuffle=False, drop_last=True
        )
        y_train_loader = DataLoader(
            y_train, batch_size=self.batch_size, shuffle=False, drop_last=True
        )
        ...
        for epoch in range(self.num_epochs):
            for (x_batch, y_batch) in zip(x_train_loader, y_train_loader):
                ...
                train_loss = self.criterion(
                    output, func.make_long_variable(y_batch).squeeze()
                )
            ...
            if (self.if_print is True) and (epoch % (self.num_epochs * self.print_freq) == 0):
                ...
                print(
                    'Validation: epoch [{}/{}], loss:{:.4f}'.format( # pylint: disable=C0209
                        epoch + 1, self.num_epochs, val_loss[-1]
                    )
                )
