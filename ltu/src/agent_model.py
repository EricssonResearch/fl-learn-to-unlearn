"""Modules for construction of the agent models."""
...
class AgentModel(object):
    ...
    def _normalize(self, x, y):
        if self.validation_ratio is None:
            ...
            x_train, _, _, _ = train_test_split(
                x, y, test_size=self.validation_ratio, shuffle=True
            )
        if self.data_normalizer.fit_flag is False:
            ...
            setattr(self.data_normalizer, 'fit_flag', True)
    ...
    def fit_predict_evaluate(self, x_train, y_train, x_test, y_test):
        """Perform fit, predict and evaluate."""
...
class CentralModel(AgentModel):
    """Central model with the same class signature as Agent model."""
